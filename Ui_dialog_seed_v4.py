...
LastEditTime: 2021-04-29 14:04:25
...
from PyQt5.QtWidgets import QMessageBox, QHeaderView, QDialog
...
class Ui_Dialog(QDialog):
    ...
    def setTableContent(self, newDict):
        ...
        self.structTable.horizontalHeader().setSectionResizeMode(0, QHeaderView.ResizeToContents)
        self.structTable.setHorizontalHeaderLabels(
            ['结构体', '成员变量', '当前值', '范围下限', "范围上限", "是否为插装变量"])
        i = 0  # 行
        j = 0  # 列
        ...
        for key, val in structDict.items():
            ...
            for key, val in val.items():
                ...
                self.structTable.setCellWidget(
                    i, 5, self.insCheckBoxItem(val['instrument'], structKey, key))
                i += 1
    # 结束
        ...
        if readJSON:
            ...
            for i in range(len(header_info)):
                ...
                for j in range(1,len(header_info[i])):
                    tempDict[header_info[i][j]] = {"value":"var1", "lower": -999, "upper": 999, "instrument": False,}
